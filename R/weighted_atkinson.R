...
run_weighted_atkinson <- function(
  ...
  assertthat::assert_that(
    epsilon > 0,
    msg = "Argument 'epsilon' must be greater than 0."
  )
  ...
  assertthat::assert_that(
    ...
    msg = glue::glue(
      "Variable '{var_name}' could not be found in the datasets."
    )
  )
  ...
  if (!is.null(wgt_name)) {
    assertthat::assert_that(
      ...
      msg = glue::glue(
        "Weight variable '{wgt_name}' could not be found in the datasets."
      )
    )
  }
  ...
  if (length(neg_datasets) > 0) {
    stop(glue::glue(
      "Negative values found in '{var_name}' for the following datasets: {paste(neg_datasets, collapse = ', ')}. Atkinson index is undefined for negative values."
    ))
  }

  output_run_weighted_atkinson <- purrr::imap(
    data_list,
    ~ {
      var <- .x[[var_name]]
      ...
      if (any(var < 0)) {
        stop(glue::glue(
          "Negative values found in '{var_name}'. Atkinson index is undefined for negative values."
        ))
      }
      ...
      if (epsilon >= 1 && any(var == 0)) {
        warning(glue::glue(
          "Zero values in '{var_name}' were removed because epsilon >= 1."
        ))
        keep <- var != 0
        var <- var[keep]
        wgt <- wgt[keep]
  ...
  output_run_weighted_atkinson <- lissyrtools::convert_list_from_ccyy_to_cc_names_yyyy(
    output_run_weighted_atkinson
  )
